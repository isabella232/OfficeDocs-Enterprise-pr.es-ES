 ...
 **Resumen:** Entender cómo diseñar redes optimizada para cargas de trabajo en Microsoft Azure IaaS.
...
Vea la sección **Pasos para preparar la red para los servicios en la nube de Microsoft** en [Elementos comunes de conectividad de Microsoft Cloud](common-elements-of-microsoft-cloud-connectivity.md).
...
|Basada solo en la nube e interconectada  <br/> |Private arbitrario, pero no se solapan con las demás conectado VNets  <br/> |
...
Estos son los pasos y ejemplo de determinar el prefijo de dirección de la subred de puerta de enlace:

1. Decidir el tamaño de la subred de puerta de enlace. Para nuestro ejemplo, hemos elegido /28.
2. Establecer los bits en la parte variable del espacio de direcciones VNet (b) en 0 para la puerta de enlace de bits de subred (G), en caso contrario, 1 (V). Para nuestro ejemplo, estamos utilizando el espacio de direcciones 10.119.0.0/16 para el VNet.<br/>
<br/>10.119. bbbbbbbb. bbbbbbbb<br/>10.119. VVVVVVVV. VVVVGGGG<br/>10.119. 11111111. 11110000<br/><br/>
3. Convertir el resultado del paso 2 en decimal y express como un espacio de direcciones. Para nuestro ejemplo, 10.119. 11111111. 11110000 es 10.119.255.240, y con la longitud del prefijo desde el paso 1, (28 en nuestro ejemplo), el prefijo de dirección de subred de puerta de enlace resultante es 10.119.255.240/28.
...
Para obtener más información acerca de la cantidad máxima de máquinas virtuales en una subred o VNet, vea [Los límites de la red](https://docs.microsoft.com/azure/azure-subscription-service-limits#networking-limits).
...
Para obtener más información, vea [La resolución de nombres para las máquinas virtuales y las instancias de función](https://docs.microsoft.com/azure/virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances).
...
La figura 4 muestra un equilibrador de carga externo en Azure que distribuye el tráfico entrante en una regla de NAT de entrada o extremo a un conjunto de máquinas virtuales en un conjunto con equilibrio de carga.
...
La figura 5 muestra un equilibrador de carga interno en Azure que distribuye el tráfico entrante en una regla de NAT de entrada o extremo a un conjunto de máquinas virtuales en un conjunto con equilibrio de carga.
...
Para obtener más información, vea [rutas de definida por el usuario y el reenvío de IP](https://docs.microsoft.com/azure/virtual-network/virtual-networks-udr-overview).
...
Para obtener más información, vea [Seguridad de la nube de Microsoft para arquitectos de empresa](https://aka.ms/cloudarchsecurity) y [Seguridad de red de Azure](https://azure.microsoft.com/blog/azure-network-security/).
...
La figura 8 muestra cinco VNets conectados en serie con una configuración encadenados.
...
|:-----|:-----|
|VPN sitio a sitio (S2S)  <br/> |Conexión de sitios de 1-10 (incluidos otros VNets) a un único VNet.  <br/> |
...
Para obtener más información sobre el número máximo de conexiones, vea [Los límites de la red](https://docs.microsoft.com/azure/azure-subscription-service-limits#networking-limits).
...
Para obtener más información acerca de VNet interconexión, vea [interconexión VNet](https://docs.microsoft.com/azure/virtual-network/virtual-network-peering-overview).
...
La figura 11 muestra un VNet con los cuatro tipos de conexiones: una conexión de P2S desde un equipo, una conexión VPN S2S desde una red local, una conexión de ExpressRoute desde una red local y una conexión de VNet a VNet desde otro VNet. 
...
Para obtener más información, vea [acerca de VPN puerta de enlace](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateways).
  
### <a name="step-3-add-routes-to-your-intranet-to-make-the-address-space-of-the-vnet-reachable"></a>Paso 3: Agregar rutas a la intranet para hacer que el espacio de direcciones de la VNet sean accesibles.
...
Este es un ejemplo de la definición de los prefijos para el espacio de direcciones de red Local alrededor del espacio de direcciones "hueco" creado por el VNet:
...
|2. los prefijos no se superponen para variables octetos hasta, pero sin incluir el último octeto usado en el espacio de direcciones VNet de lista.  <br/> |10.0.0.0/16, 10.1.0.0/16... 10.99.0.0/16, 10.101.0.0/16... 10.254.0.0/16, 10.255.0.0/16 (255 prefijos, se omitirá 10.100.0.0/16)  <br/> |
|3. los prefijos no se superponen en el último octeto usado del espacio de direcciones VNet de lista.  <br/> |10.100.0.0/24, 10.100.1.0/24... 10.100.99.0/24, 10.100.101.0/24... 10.100.254.0/24, 10.100.0.255.0/24 (255 prefijos, se omitirá 10.100.100.0/24)  <br/> |
...
La ruta de sistema predeterminada para subredes Azure apunta a Internet. Para asegurarse de que todo el tráfico de las máquinas virtuales se transmite a través de la conexión entre locales, cree una tabla de enrutamiento con la ruta predeterminada que usa la puerta de enlace Azure como su dirección del próximo salto. A continuación, asocie la tabla de rutas con la subred. Esto se conoce como forzar tunelización. Para obtener más información, vea [Configure había forzado tunelización](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-forced-tunneling-rm).
...
La figura 18 muestra una VNet entre locales con una ruta definida por el usuario para una subred que señala a la puerta de enlace de Azure.
  
## <a name="sharepoint-server-2016-farm-in-azure"></a>Granja de servidores de SharePoint Server 2016 en Azure
<a name="cross_prem"> </a>

Un ejemplo de una intranet hospedado en Azure IaaS de carga de trabajo de TI es una granja de servidores de SharePoint Server 2016 altamente disponibles, de varios nivel.
